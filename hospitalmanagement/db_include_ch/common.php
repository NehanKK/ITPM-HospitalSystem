...
function getDetailsByWhereCondition($table_name,$whe,$select_fld){
    include("config.php");
    $dbObj=new mySqlDB;
    $dbObj->connect($hostname,$username,$password);
    $dbObj->dBSelect($dbname);
    $DData=$dbObj->querySelect($table_name,array($select_fld),array($select_fld),"A",$whe);
    return stripslashes($DData[0][0]);
}


function getSumByTableNameAndColumnNameAndWhere($tb,$clm,$whe) {
	include("config.php");
	$dbObj=new mySqlDB;
	$dbObj->connect($hostname,$username,$password);
	$dbObj->dBSelect($dbname);
	$query = "SELECT SUM($clm) FROM $tb WHERE $whe"; 
	//echo "<br/><br/> $query";
...
function getCountByTableNameAndColumnName($tb,$clm,$whe) {
	include("config.php");
	$dbObj=new mySqlDB;
	$dbObj->connect($hostname,$username,$password);
	$dbObj->dBSelect($dbname);
	$query = "SELECT COUNT($clm) FROM $tb WHERE $whe"; 
	//echo "<br/> $query";
	$result = mysql_query($query) or die(mysql_error());
	$row = mysql_fetch_array($result);
	return $row['0'];
